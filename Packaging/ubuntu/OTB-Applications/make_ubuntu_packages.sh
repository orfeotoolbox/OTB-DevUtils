...
This script is used to automate the OTB applications packaging for Ubuntu.
Source packages are created in ${TMPDIR} directory.
...
echo "Archive export..."
cd "$topdir"
hg archive -r "$revision" -t tgz "$TMPDIR/otbapp-$otb_version_full.tar.gz"

echo "Archive extraction..."
cd "$TMPDIR"
tar xzf "otbapp-$otb_version_full.tar.gz"
mv "otbapp-$otb_version_full.tar.gz" "otbapp_$otb_version_full.orig.tar.gz"
...
#for target in lucid natty oneiric ; do
for target in lucid natty ; do
