#!/bin/bash
# Script to automate the Ossim library packaging for Ubuntu.
#
...
TMPDIR=$(mktemp -d /tmp/ossim.XXXXXX)
DIRNAME=$(dirname $0)
if [ "${DIRNAME:0:1}" == "/" ] ; then
    CMDDIR=$DIRNAME
...
This script is used to automate the Ossim library packaging for
...
Options:
  ...
  -a archive    Archive containing Ossim orig sources
...
set_ubuntu_code_name ()
{
    case "$1" in
        ...
        "vivid" )
            ...
            ;;
        "utopic" )
            ubuntu_codename="Utopic Unicorn"
...
changelog_version=`grep -E -e 'ossim \(.+-.+\)' "$DEBDIR/changelog.in" | head -n 1 | cut -d '(' -f 2 | cut -d '-' -f 1`
...
cd "$TMPDIR/ossim-${changelog_version}"
cp -a "$DEBDIR" debian

echo "Source package generation..."
first_pkg=1
for target in precise trusty vivid utopic; do
    ...
    echo "Ossim source package for Ubuntu $ubuntu_codename is available in $TMPDIR"
done
