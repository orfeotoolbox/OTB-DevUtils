#!/bin/bash
# Script to automate the OpenJPEG packaging for Ubuntu.
#
...
TMPDIR=$(mktemp -d /tmp/openjpeg.XXXXXX)
DIRNAME=$(dirname $0)
if [ "${DIRNAME:0:1}" == "/" ] ; then
    CMDDIR=$DIRNAME
...
This script is used to automate the OpenJPEG library packaging for
...
Options:
  ...
  -a archive    Archive containing OpenJPEG orig sources
...
Example:
  ./make_ubuntu_packages.sh -a openjpeg-2.1.0.orig.tar.gz -p 1
...
changelog_version=`grep -E -e 'openjpeg2 \(.+-.+\)' "$DEBDIR/changelog.in" | head -n 1 | cut -d '(' -f 2 | cut -d '-' -f 1`
...
cd "$TMPDIR/openjpeg-${changelog_version}"
cp -a "$DEBDIR" debian
...
for target in precise trusty ; do
    ...
    if [ "${target}" == "precise" ] ; then
        rm -f debian/libopenjpip-server.apache2
        rm -f debian/libopenjpip-server.conf
        cp -f "$DEBDIR/libopenjpip-server.postinst.precise" debian/libopenjpip-server.postinst
        cp -f "$DEBDIR/libopenjpip-server.postrm.precise" debian/libopenjpip-server.postrm
    else
        cp -f "$DEBDIR/libopenjpip-server.apache2" ./debian
        cp -f "$DEBDIR/libopenjpip-server.conf" ./debian
        rm -r debian/libopenjpip-server.postinst
        rm -f debian/libopenjpip-server.postrm
    fi
    ...
    echo "OpenJPEG source package for Ubuntu $ubuntu_codename is available in $TMPDIR"
done
