...
TMPDIR=$(mktemp -d /tmp/muparserx.XXXXXX)
DIRNAME=$(dirname $0)
if [ "${DIRNAME:0:1}" == "/" ] ; then
    CMDDIR=$DIRNAME
...
This script is used to automate the muParserX library packaging for
...
Options:
  ...
  -a archive    Archive containing muParserX orig sources
...
changelog_version=`grep -E -e 'muparserx \(.+-.+\)' "$DEBDIR/changelog.in" | head -n 1 | cut -d '(' -f 2 | cut -d '-' -f 1`
...
cd "$TMPDIR/muparserx-${changelog_version}"
cp -a "$DEBDIR" debian
...
for target in precise trusty vivid ; do
    ...
    echo "muParserX source package for Ubuntu $ubuntu_codename is available in $TMPDIR"
done
