...
Copyright (C) 2010, 2011 CNES (Centre National d'Etudes Spatiales)
by Sebastien DINOT <sebastien.dinot@c-s.fr>
modifications by Julien MALIK <julien.malik@c-s.fr>
...
This script is used to automate the Monteverdi2 workshop packaging for
...
echo "Archive export..."
cd "$topdir"
hg archive -r "$revision" -t tgz "$TMPDIR/monteverdi2-$src_version_full.tar.gz"

echo "Archive extraction..."
cd "$TMPDIR"
tar xzf "monteverdi2-$src_version_full.tar.gz"
mv "monteverdi2-$src_version_full.tar.gz" "monteverdi2_$src_version_full.orig.tar.gz"
